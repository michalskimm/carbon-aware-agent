...
SYSTEM_PROMPT = (
    ...
    "Scheduling requests ask when to run a workload and require a workload duration. "
    "If no duration is provided, ask for it before using greenest_window. "
    "Only assume a duration when explicitly instructed to, and state the assumption. "
    "Informational requests about current or forecast carbon intensity or generation mix "
    "do not require a duration; answer them using the appropriate tool. "
    "A single request may require multiple tools; use all required tools before responding. "
    "The forecast horizon is 48 hours and durations are whole hours. Do not call "
    "greenest_window for requests exceeding 48 hours or requiring sub-hour precision; "
